...
config = {
    'apiKey': "AIzaSyAa4mwANCFq6TdPA-H_sVDb04H1JgvLMms",
    'authDomain': "cnsproject-889dd.firebaseapp.com",
    'databaseURL': "https://cnsproject-889dd-default-rtdb.firebaseio.com/",
    'projectId': "cnsproject-889dd",
    'storageBucket': "cnsproject-889dd.appspot.com",
    'messagingSenderId': "978128836170",
    'appId': "1:978128836170:web:c0d0ff5b9102ff4fdc78b3",
}
...
def postreport(request):
    ...
    user_ = user_[0]
    user_ = user_['localId']
    database.child('users').child(user_).child(
        'reports').child(time_).set(data)
    name = database.child('users').child(user_).child(
        'details').child('name').get().val()
    print(name)
    return render(request, 'postin.html', {'email': name})
...
def check(request):
    ...
    timestamps = database.child('users').child(a).child(
        'reports').shallow().get().val()  # list of timestamps
    if timestamps != None:
        lis_time = []
        for i in timestamps:
            lis_time.append(i)
        ...
        work = []
        for i in lis_time:
            wor = database.child('users').child(a).child(
                'reports').child(i).child('title').get().val()
            work.append(wor)
        date = []
        for i in lis_time:
            i = float(i)
...
def post_check(request):
    ...
    a = a['localId']
    print(a)
    work = database.child('users').child(a).child(
        'reports').child(time).child('title').get().val()
    progress = database.child('users').child(a).child(
        'reports').child(time).child('details').get().val()
    url = database.child('users').child(a).child(
        'reports').child(time).child('url').get().val()
    print(work)
    # i =float(time)
    # dat = datetime.datetime.fromtimestamp(i).strftime('%H:%M %d-%m-%Y')
    name = database.child('users').child(a).child(
...
def fileUpload(request):
    ...
    n1 = name.split('/')[-1]
    try:
        err = pyAesCrypt.encryptFile(
            "media/"+n1, "media/"+n1+".aes", password, bufferSize)
        print(err)
    except:
...
def to_decrypt(request):
    ...
    for ex in extentions:
        if n1.find("."+ex) != -1:
            ext = ex
            decrypt = pyAesCrypt.decryptFile(
                "media/"+n1, "media/"+n1+"."+ex, password, bufferSize)
